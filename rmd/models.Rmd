...
$$
f(x\vert\vartheta) = h(x)\exp[\eta \cdot T(x) - A(\vartheta)],
$$
...
Chosen priors do not contribute strongly to the posterior, so the data can "speak for itself". Algebraically then, the model is given as
\begin{align*}
...
```{r}
# extract the log-posterior and NUTS parameters for the pooled model
lp_wm <- log_posterior(weibull_model)
np_wm <- nuts_params(weibull_model)
# and for the censored and hierarchical models
lp_cwm <- log_posterior(weibull_cens)
np_cwm <- nuts_params(weibull_cens)
lp_whm <- log_posterior(weibull_hier)
np_whm <- nuts_params(weibull_hier)
```
